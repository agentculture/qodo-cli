...
qodo-cli vendors its `.claude/skills/` from **guildmaster** — the
...
Every vendored `SKILL.md` carries `type: command`. qodo-cli
...
#   - consumer-identifying prose: `guildmaster` → `qodo-cli` (NOT
...
If a re-sync would lose a qodo-cli adaptation, lift the change
...
#     → `which qodo-cli vendors from guildmaster`
