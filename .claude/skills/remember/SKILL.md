...
description: >
  ...
  defaults records to this agent's PERSONAL, PRIVATE scope (`--scope qodo-cli
...
scope:** the default personal scope is **private** (`--scope qodo-cli
...
- `--scope NAME` / `--visibility public|private` — record scope. **The wrapper
  ...
  (here, `qodo-cli`). Private records are served only to a recall in the same
